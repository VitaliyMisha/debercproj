...
Верифікація: lint ✅, type-check ✅, 176 тестів ✅, build ✅. PWA precache 655.83 → **698.16 KiB** (31 entries) — ріст від нових версій React/Firebase, не від нашого коду.

Процес: оновлено в worktree, злито в `main` локально (fast-forward), потім `git push origin main`.

### Екран правил у застосунку (2026-09-08)

Змерджено в `main` (`76e8e61`), робочу гілку видалено. Тестів: 168 → **176**.

Обрано через brainstorming: показано 6 візуальних варіантів довідки (artifact із мокапами в палітрі гри), користувач обрав **повний документ** — максимум інформації, попри озвучений компроміс «довго читати посеред роздачі». Щоб довжина не заважала, документ отримав **зміст-якорі**: чіпи розділів угорі скролять до потрібного місця.

**`RulesSheet.tsx`** (новий) поверх наявного `BottomSheet`. 8 розділів у порядку `GAME_RULES.md`: гравці та стіл → введення рахунку → Б → ХВ → ВіС → кінець гри → дилер → окремі випадки.

**Ключове архітектурне рішення — числа не дублюються.** Проза правил лежить в i18n (`rules.*`, uk + en), але штрафи й цільовий рахунок підставляються інтерполяцією з активних `gameRules`/`targetScore` (`{{secondB}}`, `{{hv}}`, `{{target}}`). Інакше довідка стверджувала б «−100» після того, як гравець змінив штраф у `PenaltySheet`. Це створює друге джерело правди для ПРОЗИ — тому в `GAME_RULES.md` і `CLAUDE.md` додано попередження оновлювати обидва місця; числа розійтися не можуть за побудовою.

**`allowVis: false` ховає ВіС повністю**, а не лише його розділ: у локалі едж-кейси розділено на `edge.body` (загальні) і `edge.visBody` (про ВіС), а перелік токенів має два варіанти (`input.tokens` / `input.tokensNoVis`). Фільтрувати текст пошуком слова «ВіС» було б крихко.

**Підключення без змін в `App.tsx`** (крім передачі `gameRules` у `GameHeader`): обидва входи тримають шит **локальним станом**, як це вже роблять `PenaltySheet` у `SetupScreen` і `ConfirmSheet` у `GameHeader`. Кнопка названа **«Як грати»**, бо секція `setup.rulesLabel` «Правила» — це вже налаштування штрафів; сам документ лишився «Правила гри».

**Тести** (`tests/ui/rulesSheet.test.tsx`, 8): порядок розділів, підстановка змінених штрафів (−50/−25 замість дефолтних), інтерполяція `targetScore`, приховування ВіС разом з едж-кейсами й токеном, зміст без ВіС-запису, скрол за якорем, закриття. **Чесність перевірено**: прибирання фільтра `allowVis` валить 2 тести, хардкод `-100` замість інтерполяції — 1.

**Дрібниці по дорозі:**
- Проєкт **не використовує** `@testing-library/jest-dom`, попри його наявність у devDeps — усі тести на `toBeTruthy()`/`toBeNull()`. Новий тест дотримується цього ж патерну.
- i18next-типи відкидають динамічний ключ разом із `returnObjects` (вимагають `defaultValue: string`) — виклик звужено локально, рантайм захищає `Array.isArray`.
- `useCallback` навколо `paragraphs` прибрано: функція викликається просто в рендері, мемоізація нічого не давала (Biome це і виявив).
- Верифіковано у справжньому браузері (Playwright, 390×844) обома мовами: скрол, якорі, підстановка чисел. Побічно з'ясувалось, що **worktree без `.env.local` не монтує застосунок** (Firebase FATAL) — файл gitignored, для локальної перевірки його треба копіювати вручну.

Верифікація: lint ✅, type-check ✅, 176 тестів ✅, build ✅ (+13 KiB precache — тексти двома мовами).
...
Усе змерджено в `main` двома fast-forward-пушами (`a481c7b..f41e37b`, далі `f41e37b..21ab81b`), Vercel задеплоїв автоматично. Локальний `main` синхронізовано, робочий worktree `deps-update-check` видалено; віддалену гілку `worktree-deps-update-check` теж видалено з GitHub (була повністю в `main`). Лишилась єдина гілка — `main`.
