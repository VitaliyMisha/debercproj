import type { Game, GameRulesConfig, Round, SavedGameState } from '../types';
...
export const isValidScore = (val: string | number, gameRules?: GameRulesConfig): boolean => {
  ...
  return /^-?\d+$/.test(trimmed) || trimmed === 'Б' || trimmed === 'ХВ' || (allowVis && trimmed === 'ВІС');
};

export const parseScore = (value: string | number, pid: string, playerRounds: Round[], gameRules?: GameRulesConfig): number | string => {
 ...
 */
export const getVisDisplayValue = (roundIndex: number, playerId: number, rounds: Round[], gameRules: GameRulesConfig): string | number => {
  const pid = String(playerId);
  const val = rounds[roundIndex]?.scores[pid];
  const isVis = (v: unknown): boolean => typeof v === 'string' && v.toUpperCase() === 'ВІС';
 ...
 */
export const bestOpponent = (scores: Round['scores'], playerId: number): { playerId: number; score: number } =>
...
export const calculateGameTotals = (game: Game, gameRules: GameRulesConfig): Record<number, number> => {
  ...
  game.rounds.forEach((round, idx, allRounds) => {
    ...
    stillPendingVis.forEach(({ playerId: visPlayerId, roundIndex }) => {
      ...
      const hangingScore = Math.max(...Object.values(prevRound.scores).map((val) => (typeof val === 'number' ? val : 0)));
      ...
      if (visScore > bestOpponentCurrent.score) {
        ...
        pendingVis.splice(
          pendingVis.findIndex((p) => p.playerId === visPlayerId && p.roundIndex === roundIndex),
          1
        );
...
export function savePlayerNames(newNames: string[]): void {
  ...
  const existing = stored.filter((e) => !trimmed.some((n) => n.toLowerCase() === e.toLowerCase()));
