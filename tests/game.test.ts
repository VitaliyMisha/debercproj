...
import type { Game, GameRulesConfig, Player, Round } from '../src/types';
...
const mkPlayers = (...names: string[]): Player[] => names.map((name, i) => ({ id: i + 1, name, winCount: 0 }));
  ...
  describe('calculateGameTotals', () => {
    ...
    it('check "ХВ" logic', () => {
      const game: Game = {
        id: 1,
        players,
        rounds: [{ id: 1, number: 1, scores: { '1': 'ХВ', '2': 20 } }],
        dealerId: 1,
        createdAt: '',
      };
...
describe('single Б per round — engine behaviour', () => {
  ...
  it('two players with Б in same round: each has independent counter, no penalty for first', () => {
    const g = mkGame([p1, p2], [mkRound(1, { '1': 'Б', '2': 'Б' })]);
...
describe('undo last round — calculateGameTotals after rounds.slice(0,-1)', () => {
  ...
  it('basic numeric scores: totals revert to state before last round', () => {
    const g = mkGame([p1, p2], [mkRound(1, { '1': 100, '2': 50 }), mkRound(2, { '1': 200, '2': 60 }), mkRound(3, { '1': 300, '2': 70 })]);
  ...
  it('undo round with first Б: player total reverts to 0', () => {
    const g = mkGame([p1, p2], [mkRound(1, { '1': 'Б', '2': 80 })]);
  ...
  it('undo round with second Б: -100 penalty disappears', () => {
    const g = mkGame([p1, p2], [mkRound(1, { '1': 'Б', '2': 50 }), mkRound(2, { '1': 'Б', '2': 30 })]);
  ...
  it('undo ВіС resolution round: winner bonus removed, ВіС treated as unresolved (0)', () => {
    ...
    const g = mkGame([p1, p2], [mkRound(1, { '1': 50, '2': 'ВІС' }), mkRound(2, { '1': 40, '2': 60 })]);
  ...
  it('undo ВіС loss round: Б penalty removed from ВіС player', () => {
    ...
    const g = mkGame([p1, p2], [mkRound(1, { '1': 100, '2': 'ВІС' }), mkRound(2, { '1': 70, '2': 30 })]);
  ...
  it('undo round that contained a new ВіС: pending entry removed', () => {
    ...
    const g = mkGame([p1, p2], [mkRound(1, { '1': 100, '2': 200 }), mkRound(2, { '1': 'ВІС', '2': 50 })]);
  ...
  it('undo only round: all totals reset to 0', () => {
    const g = mkGame([p1, p2], [mkRound(1, { '1': 300, '2': 150 })]);
    const after = calculateGameTotals({ ...g, rounds: [] }, rules);
    expect(after[1]).toBe(0);
    expect(after[2]).toBe(0);
  });
